...
// Reads currency symbol and dark mode from global ThemeContext
...
export default function ExpensePieChart({ data }) {
  ...
  const options = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        callbacks: {
          label: (ctx) =>
            ` ${displayCurrency}${ctx.parsed.toLocaleString()} (${total > 0 ? Math.round((ctx.parsed / total) * 100) : 0}%)`
        },
      },
    },
  };
  ...
  return (
    <div className={`${isDark ? "bg-gray-900 border-gray-800" : "bg-white border-gray-100"} border rounded-2xl p-6 shadow-xl flex flex-col transition-colors duration-300`}>
      <h3 className={`font-bold text-sm mb-1 ${isDark ? "text-gray-300" : "text-gray-700"}`}>
